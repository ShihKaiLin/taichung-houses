...
# ============================================================
# 2. 品牌質感法律頁尾 (Branding & Legal Footer)
# ============================================================
LEGAL_FOOTER = """
...
# ============================================================
# 3. 頁面標頭生成 (SEO & Script Generation)
# ============================================================
def get_head(title, desc="", img="", is_home=False, map_data_json="[]"):
    ...
    ga = f"""<script async src="https://www.googletagmanager.com/gtag/js?id={GA4_ID}"></script><script>window.dataLayer=window.dataLayer||[];function gtag(){{dataLayer.push(arguments);}}gtag('js',new Date());gtag('config','{GA4_ID}');</script>""" if GA4_ID else ""
    
    # 結構化數據 (JSON-LD) 提升 Google 搜尋排名
    json_ld = f"""<script type="application/ld+json">{{"@context":"https://schema.org","@type":"RealEstateAgent","name":"{SITE_TITLE}","telephone":"{MY_PHONE}","url":"https://shihkailin.github.io/taichung-houses/"}}</script>"""
                            ...
                            marker.addListener("click", () => {{
                                const content = `
                                    <div style="padding:10px;width:200px;font-family:sans-serif;">
                                        <div style="background:url('${{loc.img}}') center/cover;height:100px;border-radius:8px;margin-bottom:10px;"></div>
                                        <h4 style="margin:0;color:#1A365D;font-size:14px;line-height:1.4;">${{loc.name}}</h4>
                                        <div style="color:#C5A059;font-weight:900;font-size:16px;margin:5px 0 10px;">${{loc.price}}</div>
                                        <a href="${{loc.url}}" style="display:block;text-align:center;background:#1A365D;color:#fff;text-decoration:none;padding:10px;border-radius:8px;font-size:12px;font-weight:bold;">查看詳細建議</a>
                                    </div>`;
    ...
    <style>
        ...
        .hero-content{{position:relative;z-index:2;text-align:center;}}.hero-content h2{{font-size:34px;margin:0;letter-spacing:6px;font-weight:900;text-shadow:0 2px 10px rgba(0,0,0,0.3);}}
        ...
        .tag{{padding:12px 22px;border-radius:50px;background:var(--sk-light);font-size:13px;color:#4A5568;cursor:pointer;white-space:nowrap;border:1px solid #edf2f7;font-weight:600;transition:0.3s cubic-bezier(0.4, 0, 0.2, 1);}}
        .tag.active{{background:var(--sk-navy);color:#fff;border-color:var(--sk-navy);box-shadow:0 5px 15px rgba(26,54,93,0.2);}}
        .property-card{{margin:35px 20px;border-radius:28px;overflow:hidden;background:#fff;box-shadow:var(--sk-shadow);border:1px solid #f1f5f9;transition:0.3s;}}
        ...
        .btn-ext{{display:block;text-align:center;padding:18px;background:#fff;color:var(--sk-navy);text-decoration:none;border-radius:16px;margin-top:15px;font-weight:800;border:2px solid #edf2f7;transition:0.3s;}}
...
def build():
    ...
    for i, row in enumerate(reader):
        ...
        internal_url = f"./{slug}/"
        map_data.append({"name":name, "price":p_str, "img":img, "url":internal_url, "address":search_addr})
        ...
        detail = f"""<div class="container">
            ...
            <div style="padding:45px 25px;background:#fff;border-radius:45px 45px 0 0;margin-top:-55px;position:relative;">
                ...
                <div class="advice-box">
                    ...
                    此物件所處地段極具保值性。若您對本案感興趣，建議直接點擊下方 LINE 諮詢，我將為您提供該社區最新的成交行情分析與最優惠的貸款方案建議。
                </div>
                {ext_btn}
                <a href="https://www.google.com/maps/search/?api=1&query={urllib.parse.quote(search_addr)}" target="_blank" style="display:block;text-align:center;padding:20px;background:var(--sk-navy);color:#fff;text-decoration:none;border-radius:18px;margin-top:15px;font-weight:800;font-size:15px;letter-spacing:1px;">📍 在地圖上開啟導航位置</a>
                {LEGAL_FOOTER}
            </div>
        ...
        (out/slug/"index.html").write_text(f"<!doctype html><html lang='zh-TW'>{get_head(name + ' | ' + reg + '房產專家推薦', d.get('描述',''), img)}<body>{detail}</body></html>", encoding="utf-8")
        
        price_val = "".join(filter(str.isdigit, p_str)) or "0"
        items.append(f'''<div class="property-card" data-region="{esc(reg)}" data-type="{esc(d.get("用途","住宅"))}" data-price="{price_val}">
            ...
            <div class="card-info" style="padding:28px;">
                ...
                <div style="font-size:13px;color:#94a3b8;margin-top:8px;">{esc(reg)} · {esc(d.get("用途","住宅"))}</div>
