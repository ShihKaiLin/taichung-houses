...
def get_head(title, ga_id, is_home=False, map_data=None):
    ...
    <head>
        ...
        <style>
            ...
            .hero {{ position: relative; height: 320px; background: url('{GITHUB_IMG_BASE}hero_bg.jpg') center/cover; display: flex; align-items: center; justify-content: center; color: #fff; }}
...
def build():
    ...
    for i, row in enumerate(reader):
        ...
        target = 'target="_blank"' if ext_url.startswith("http") else ""
        items.append(f'''
            <div class="property-card" data-region="{esc(reg)}" data-type="{esc(type_val)}" data-price="{price_val}">
                ...
                <div class="card-info">
                    ...
                    <a href="{final_url}" {target} style="display:block; text-align:center; margin-top:15px; padding:14px; background:#f8fafc; color:var(--sk-navy); text-decoration:none; font-size:13px; font-weight:700; border-radius:12px;">查看完整資訊</a>
                </div>
            </div>
        ''')
