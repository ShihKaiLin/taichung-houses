...
GA4_ID = "G-B7WP9BTP8X" # 您的 GA4 評估 ID

def esc(s): return html.escape(str(s or "").strip())

def get_final_img_url(url):
    """處理 GitHub 本地圖床與 Google Drive 轉換"""
    url = str(url).strip()
...
def build():
    ...
    for i, row in enumerate(reader):
        ...
        detail_html = f"""
        <div class="container">
            <img src="{img}" style="width:100%; height:380px; object-fit:cover;" onerror="this.src='https://placehold.co/800x600?text=圖片載入中'">
            <div style="padding:30px 25px; margin-top:-35px; background:#fff; border-radius:30px 30px 0 0; position:relative;">
                ...
                <a href="{map_link}" target="_blank" class="btn btn-map">📍 開啟 Google 地圖導覽</a>
    ...
    # 生成區域截流頁面 (/area/ 與 /sell/)
    for area, cards in area_map.items():
        a_slug = urllib.parse.quote(area)
        for d_path, title_p, insight in [
            ("area", "買房推薦", f"📍 {area} 區域專家分析：本區生活機能完善，值得關注。"),
            ("sell", "委託賣房", f"🏠 為什麼選擇林世塏？我們透過 AI 讓您的物件獲得最高曝光。")
        ]:
